...
const createAndroidNotificationChannel = () => {
  try {
    ...
    const channel = new firebase.notifications.Android.Channel(
      ...
      .setDescription('agzotic app channel')
...
const clearNotifications = () => {
  ...
  firebase.notifications().cancelNotification('agzotic');
  firebase.notifications().removeDeliveredNotification('agzotic');
};
// Token Listeners
  ...
  const onNotificationCallback = async (notification) => {
    ...
    if (Platform.OS === 'android') {
      notification.android
        .setAutoCancel(true)
        .android.setColor('#f57d02')
        .android.setSmallIcon('ic_notification')
        .android.setChannelId('agzotic')
