...
// Components
import OrderCompleteTabComponet from '../components/OrderCompleteTabComponet';
...
export default class OrderCompleteTab extends Component {
  ...
  fetchViewOrders = async () => {
    try {
      ...
      // if (userInfo) {
      const params = {
        status: JSON.stringify(['delivered']),
      };
  ...
  renderItem = ({item}) => (
    <OrderCompleteTabComponet
      item={item}
      nav={this.props.navigation}
      handleRateProduct={this.handleRateProduct}
    />
  );
  ...
  handleRateProduct = async (ratings, orderId) => {
    try {
      // starting loader
